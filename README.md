...
- yDayColor：对象Object，可选 —— ```DateColor:{day:'2021-5-1',serene:'serene'}```，第一个参数是当前签到日期，第二个是颜色对象中的键名
- yEmotions：对象Object，可选 —— 颜色映射表。```colors: {serene: '#64d9fe'}```，使用yDayColor和yEmotions时注意场景，比如你想要获取用户“开心”心情时标注不同颜色在日历上，此时你要在调用组件page中添加将“开心”和colors中的“serene”对应起来，然后通过上一个参数传入自定义组件！
